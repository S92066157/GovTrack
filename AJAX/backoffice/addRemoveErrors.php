...
if (isset($_POST['error1']) && isset($_POST['error2']) && isset($_POST['error3']) && isset($_POST['error1Remark']) && isset($_POST['error2Remark']) && isset($_POST['error3Remark'])) {
    ...
    if ($textError1 == 1) {
        ...
        $resultExisting1 = mysqli_query($conn, $sqlExisting1);
            ...
            if (mysqli_num_rows($resultExisting1) > 0) {
                ...
                $sql = "INSERT INTO taskerrors(uniqueID, errorID, isErrorOccured, submittedBy, remark) VALUES (?, ?, ?, ? , ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssiss", $uID, $Error1, $textError1 , $user, $error1Remark);
    ...
    if ($textError2 == 1) {
        ...
        $resultExisting2 = mysqli_query($conn, $sqlExisting2);
            ...
            if (mysqli_num_rows($resultExisting2) > 0) {
                ...
                $sql1 = "INSERT INTO taskerrors(uniqueID, errorID, isErrorOccured , submittedBy, remark) VALUES (?, ?, ?, ?, ?)";
        $stmt1 = $conn->prepare($sql1);
        $stmt1->bind_param("ssiss", $uID, $Error2, $textError2 , $user, $error2Remark);
    ...
    if ($textError3 == 1) {
        ...
        $sql2 = "INSERT INTO taskerrors(uniqueID, errorID, isErrorOccured, submittedBy, remark) VALUES (?, ?, ?, ?, ?)";
        $stmt2 = $conn->prepare($sql2);
        $stmt2->bind_param("ssiss", $uID, $Error3, $textError3, $user, $error3Remark);
